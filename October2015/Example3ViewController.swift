...
class Example3ViewController: UIViewController, RelativePositionViewDelegate {
  ...
  func relativePositionView(relativePositionView:RelativePositionView, didMoveToRelativePosition position:CGPoint, velocity:CGPoint) {
    ...
    displayView.transform = CGAffineTransformMakeScale(position.x, position.y)
  }
  
  func nextButtonPressed(button:UIButton) {
    self.navigationController?.pushViewController(Example3aViewController(), animated: true)
  }
}
