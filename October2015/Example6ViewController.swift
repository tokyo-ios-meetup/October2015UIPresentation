...
class Example6ViewController: UIViewController, RelativePositionViewDelegate, UITextViewDelegate {
  lazy var relativePositionView : RelativePositionView = {
    let relativePositionView = RelativePositionView(frame:
      CGRectOffset(
    ...
    relativePositionView.backgroundColor = UIColor.redColor()
    relativePositionView.delegate = self
    return relativePositionView
  }()
  lazy var indicatorView : UIView = {
    let indicatorView = UIView(frame: CGRect(
      x: 0,
      y: 0,
      width: (self.textView.bounds.size.height / self.textView.contentSize.height) * self.relativePositionView.bounds.size.width,
      height: self.relativePositionView.bounds.size.height
    ))
  ...
  lazy var textView : UITextView = {
    let view = UITextView(frame:CGRect(x: 20, y: 100, width: self.view.bounds.size.width-40, height: 140))
    view.text = self.text
    view.delegate = self
    return view
  }()
  ...
  override func loadView() {
    ...
    relativePositionView.addSubview(self.indicatorView)
  }
...
//    self.navigationController?.pushViewController(Example5ViewController(), animated: true)
  ...
  func scrollViewDidScroll(scrollView: UIScrollView) {
    self.indicatorView.frame.origin.x = (scrollView.contentOffset.y / scrollView.contentSize.height) * relativePositionView.bounds.size.width - self.indicatorView.bounds.width / 2
  }
}
